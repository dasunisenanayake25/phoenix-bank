...
export class KeyCeremonyService {
   ...
   */
  public reconstructAndSign(sharesHex: string[]): { success: boolean; signedEntries: number; signatures: string[] } {
    if (!sharesHex || sharesHex.length < 3) {
      throw new BadRequestException('At least 3 shares are required to reconstruct the key.');
    }
    ...
    try {
      ...
      throw new BadRequestException(`Key reconstruction failed: ${(error as Error).message}`);
    } finally {
